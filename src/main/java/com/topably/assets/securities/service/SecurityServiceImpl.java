...
public class SecurityServiceImpl implements SecurityService {
    ...
    public Collection<SecurityDto> searchSecurities(String searchTerm, Collection<SecurityType> securityTypes) {
    ...
    @Override
    public Collection<Security> findCertainTypeOfSecuritiesByExchangeCodes(Collection<SecurityType> securityTypes,
                                                                              Collection<String> exchangeCodes) {
        Set<String> types = securityTypes.stream().map(SecurityType::name).collect(toSet());
        return securityRepository.findAllBySecurityTypesInAndExchangeCodesIn(types, exchangeCodes);
    }

    @Override
    public Security findSecurity(String ticker, String exchange) {
        return securityRepository.findByTickerAndExchange_Code(ticker, exchange);
    }
}
